...
public class Solution {
    ...
    public static void main(String args[] ) throws Exception {
        ...
        scanner.skip("(\r\n|[\n\r\u2028\u2029\u0085])?");
        int numItems = Integer.parseInt(order[0]);
        int brianItem = Integer.parseInt(order[1]);
        
        String[] billItems = scanner.nextLine().split(" ");
        scanner.skip("(\r\n|[\n\r\u2028\u2029\u0085])?");
        int[] bill = new int[numItems];
        for(int i=0; i<numItems; i++){
            bill[i] = Integer.parseInt(billItems[i]);
        }
        
        int billCharged = scanner.nextInt();
        scanner.skip("(\r\n|[\n\r\u2028\u2029\u0085])?");
        
        int billActual = calculateBill(brianItem, bill);
        
        if(billCharged == billActual){
            System.out.println("Bon Appetit");
        } else{
            System.out.println(billCharged - billActual);
        }
